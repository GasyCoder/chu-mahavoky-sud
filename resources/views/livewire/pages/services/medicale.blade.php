...
<div id="content-medical" class="tab-content">
    <!-- Introduction aux services médicaux avec design amélioré -->
    <div class="mb-5 text-center" data-aos="fade-up" data-aos-duration="1000">
        ...
        <p class="max-w-xl mx-auto text-gray-dark text-lg leading-relaxed">
    ...
    <!-- Navigation services médicaux améliorée avec bouton "Tous" -->
    <div class="relative mb-10" data-aos="fade-up" data-aos-duration="1000">
        <div class="flex overflow-x-auto py-6 scrollbar-hide service-filter-container">
            <div class="flex space-x-3 px-2 mx-auto">
                <!-- Bouton "Tous" ajouté au début -->
                ...
                <a href="#{{ $category->slug }}" class="service-filter whitespace-nowrap px-5 py-2 rounded-full border-2 border-purple/20 bg-white text-dark hover:bg-purple hover:text-white hover:border-purple transition-all duration-300 flex items-center shadow-sm">
                    <i class="{{ $category->icon ?? 'fas fa-stethoscope' }} mr-2"></i>
        ...
        <!-- Indicateurs de défilement avec style amélioré -->
        <div class="absolute left-0 top-1/2 -translate-y-1/2 hidden md:block">
            <button id="scroll-left" class="w-10 h-10 rounded-full bg-white shadow-lg flex items-center justify-center text-purple hover:bg-purple hover:text-white transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-purple/50">
                <i class="fas fa-chevron-left"></i>
            </button>
        </div>
        <div class="absolute right-0 top-1/2 -translate-y-1/2 hidden md:block">
            <button id="scroll-right" class="w-10 h-10 rounded-full bg-white shadow-lg flex items-center justify-center text-purple hover:bg-purple hover:text-white transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-purple/50">
                <i class="fas fa-chevron-right"></i>
            </button>
        </div>
    </div>

    <!-- Services médicaux avec design de carte amélioré -->
    ...
    <!-- Pagination avec design personnalisé -->
    <div class="flex justify-center mb-12" data-aos="fade-up" data-aos-duration="800">
        @if($services->hasPages())
            <nav role="navigation" aria-label="Pagination" class="flex items-center bg-white rounded-xl shadow-md p-2">
                ...
                @else
                    <a href="{{ $services->previousPageUrl() }}" class="px-3 py-2 bg-gray-100 text-gray-700 rounded-lg mr-1 hover:bg-purple hover:text-white transition-colors duration-300">
                        <i class="fas fa-chevron-left text-xs"></i>
                    </a>
                @endif
                ...
                <div class="flex md:hidden px-4 items-center">
                    <span class="font-medium text-gray-700">Page {{ $services->currentPage() }}</span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="font-medium text-gray-700">{{ $services->lastPage() }}</span>
                </div>

                <!-- Bouton suivant -->
                @if($services->hasMorePages())
                    <a href="{{ $services->nextPageUrl() }}" class="px-3 py-2 bg-gray-100 text-gray-700 rounded-lg ml-1 hover:bg-purple hover:text-white transition-colors duration-300">
